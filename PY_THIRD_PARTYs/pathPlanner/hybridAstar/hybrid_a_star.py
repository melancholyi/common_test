...
def visualize_closed_set(closed_set, x_w, y_w, resolution):
    """
    Visualize the closed_set as a heatmap.
    
    Parameters:
        closed_set (dict): The closed_set dictionary containing node indices and their costs.
        x_w (int): Width of the grid.
        y_w (int): Height of the grid.
        resolution (float): Resolution of the grid.
    """
    # Create a 2D array to store the distance values
    distance_map = np.full((y_w, x_w), np.inf)  # Initialize with infinity

    # Populate the distance_map with values from closed_set
    for node_id, node in closed_set.items():
        # Extract grid coordinates from node_id
        x, y = calc_coordinates_from_index(node_id, x_w, min_x, min_y)
        distance_map[y, x] = node.cost  # Store the cost in the distance map

    # Normalize the distance map for better visualization
    max_distance = np.max(distance_map[distance_map < np.inf])
    distance_map = np.where(distance_map == np.inf, np.nan, distance_map / max_distance)

    # Plot the heatmap
    plt.figure(figsize=(10, 10))
    heatmap = plt.imshow(distance_map, cmap='viridis_r', origin='lower', extent=[0, x_w * resolution, 0, y_w * resolution])
    plt.colorbar(heatmap, label='Normalized Distance')
    plt.title("Distance Map from Goal Node")
...
def hybrid_a_star_planning(start, goal, ox, oy, xy_resolution, yaw_resolution):
    ...
    start_time = time.perf_counter_ns()  # 开始时间（纳秒级精度）
    ...
    end_time = time.perf_counter_ns()  # 结束时间（纳秒级精度）
    elapsed_time_us = (end_time - start_time) / 1e6  # 转换为微秒
    print(f"h_dp: {elapsed_time_us:.2f} ms")

    print('len(h_dp)', len(h_dp))
...
def main():
    ...
    end_time = time.perf_counter_ns()  # 结束时间（纳秒级精度）
    elapsed_time_us = (end_time - start_time) / 1e6  # 转换为微秒
    print(f"运行时间: {elapsed_time_us:.2f} ms")
