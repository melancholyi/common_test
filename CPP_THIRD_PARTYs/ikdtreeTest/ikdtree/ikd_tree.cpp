 ...
 * @Date: 2025-03-01 11:13:08
 * @FilePath: /mesh_planner/test/customLibTest/ikdtree/ikd_tree/ikd_tree.cpp
...
//PART: IKDTree Class Public Functions
...
template <typename PointType>
void IKDTree<PointType>::setDeleteCriterionParam(float delete_param){
    delete_criterion_param = delete_param;
}

template <typename PointType>
void IKDTree<PointType>::setBalanceCriterionParam(float balance_param){
    balance_criterion_param = balance_param;
}

template <typename PointType>
void IKDTree<PointType>::setDownsampleParam(float downsample_param){
    downsample_size = downsample_param;
}

template <typename PointType>
void IKDTree<PointType>::initIKDTree(float delete_param, float balance_param, float box_length){
...
template <typename PointType>
BoxPointType IKDTree<PointType>::getRange(){
    BoxPointType range;
    if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node){
        if (Root_Node != nullptr) {
...
template <typename PointType>
void IKDTree<PointType>::rootAlpha(float &alpha_bal, float &alpha_del){
    if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node){
...
template <typename PointType>
void IKDTree<PointType>::run_operation(IKDTreeNode ** root, Operation_Logger_Type operation){
    switch (operation.op)
    {
    case ADD_POINT:      
...
template <typename PointType>
void IKDTree<PointType>::knnSearch(PointType point, int k_nearest, PointVector& Nearest_Points, vector<float> & Point_Distance, double max_dist){   
...
template <typename PointType>
void IKDTree<PointType>::boxSearch(const BoxPointType &Box_of_Point, PointVector &Storage)
{
    Storage.clear();
...
template <typename PointType>
void IKDTree<PointType>::radiusSearch(PointType point, const float radius, PointVector &Storage)
{
    Storage.clear();
...
template <typename PointType>
int IKDTree<PointType>::insertPoints(PointVector & PointToAdd, bool downsample_on){
...
template <typename PointType>
void IKDTree<PointType>::insertBoxes(vector<BoxPointType> & BoxPoints){     
...
template <typename PointType>
void IKDTree<PointType>::deletePoints(PointVector & PointToDel){        
...
template <typename PointType>
int IKDTree<PointType>::deleteBoxes(vector<BoxPointType> & BoxPoints){
    int tmp_counter = 0;
    for (int i=0;i < BoxPoints.size();i++){ 
        if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node){               
...
template <typename PointType>
void IKDTree<PointType>::acquireRemovedPoints(PointVector & removed_points){
    pthread_mutex_lock(&points_deleted_rebuild_mutex_lock); 
...
template <typename PointType>
void IKDTree<PointType>::BuildTree(IKDTreeNode ** root, int l, int r, PointVector & Storage){
    if (l>r) return;
...
template <typename PointType>
int IKDTree<PointType>::Delete_by_range(IKDTreeNode ** root,  BoxPointType boxpoint, bool allow_rebuild, bool is_downsample){   
...
template <typename PointType>
void IKDTree<PointType>::Delete_by_point(IKDTreeNode ** root, PointType point, bool allow_rebuild){   
...
template <typename PointType>
void IKDTree<PointType>::Add_by_range(IKDTreeNode ** root, BoxPointType boxpoint, bool allow_rebuild){
...
template <typename PointType>
void IKDTree<PointType>::Add_by_point(IKDTreeNode ** root, PointType point, bool allow_rebuild, int father_axis){     
    if (*root == nullptr){
        *root = new IKDTreeNode;
        InitTreeNode(*root);
        (*root)->point = point;
...
template <typename PointType>
void IKDTree<PointType>::Search(IKDTreeNode * root, int k_nearest, PointType point, MANUAL_HEAP &q, double max_dist){
    if (root == nullptr || root->tree_deleted) return;   
...
template <typename PointType>
void IKDTree<PointType>::Search_by_range(IKDTreeNode *root, BoxPointType boxpoint, PointVector & Storage){
    if (root == nullptr) return;
    Push_Down(root);       
    if (boxpoint.vertex_max[0] <= root->node_range_x[0] || boxpoint.vertex_min[0] > root->node_range_x[1]) return;
    if (boxpoint.vertex_max[1] <= root->node_range_y[0] || boxpoint.vertex_min[1] > root->node_range_y[1]) return;
...
template <typename PointType>
void IKDTree<PointType>::Search_by_radius(IKDTreeNode *root, PointType point, float radius, PointVector &Storage)
{
    if (root == nullptr)
        return;
    Push_Down(root);
...
template <typename PointType>
void IKDTree<PointType>::flatten(IKDTreeNode * root, PointVector &Storage, delete_point_storage_set storage_type){
    if (root == nullptr) return;
    Push_Down(root);
    if (!root->point_deleted) {
        Storage.push_back(root->point);
