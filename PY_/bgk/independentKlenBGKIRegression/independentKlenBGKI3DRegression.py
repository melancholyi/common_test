...
l = torch.ones(2601, dtype=torch.float32, requires_grad=True)
mu0 = torch.mean(y_train).detach()
sigma = torch.tensor(1.0, dtype=torch.float32)

if optimizer_type == 'LBFGS':
    # 使用LBFGS优化器
    optimizer = torch.optim.LBFGS([l], lr=optim_lr, max_iter=optim_maxiter)
elif optimizer_type == 'Adam':
    # 使用Adam优化器
    optimizer = torch.optim.Adam([l], lr=optim_lr)

def closure():
    optimizer.zero_grad()
    loss = negative_log_mll((l), X_train, y_train, mu0, sigma)
    loss.backward()
...
# Plot l (all ones)
# Reshape to [51, 51]
l_reshaped = l.reshape(51, 51)
# Convert to numpy array for visualization (required by matplotlib)
l_numpy = l_reshaped.detach().cpu().numpy()  # detach from computation graph and move to CPU
cax2 = axs[1].imshow(l_numpy, cmap='viridis', extent=[min_val, max_val, min_val, max_val])
