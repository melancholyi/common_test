...
N = 100
D = 2
X_train = torch.rand(N, D) * 10  # (100, 2)
true_elevation = lambda x: 2 * torch.sin(x[:, 0]) + 3 * torch.cos(x[:, 1]) + x[:, 0] + x[:, 1]
y_train = true_elevation(X_train) + torch.randn(N) * 0.01  # (100,)
...
optimizer = torch.optim.LBFGS([lambda_, l], lr=0.1, max_iter=100)
