...
def main(cfg: DictConfig) -> None:
    ...
    seeds = torch.arange(0, 2).tolist() # get seeds
    print(f'YCY-DEBUG seeds: {seeds}')
    ...
    for seed_iter in seeds: # seed loop, 0-19
        # NOTE: 2. experiment setup
        print(f'==========2. getting experiment setip, include rng, map, sensor, robot, planner')
        rng = src.utils.set_random_seed(seed_iter) # type: np.random.RandomState(seed) | rng:RandomState(MT19937)
        ...
        # pilot survey
        x_init, y_init = pilot_survey(cfg, robot, rng) # 507 507
        ...
        while True: # NOTE: loop, MaxEntropyPlanner to get the next goal and sample dataset by using sensors
            ...
            print(
                f"seed:{seed_iter} Decision epoch: {decision_epoch} | "
        ...
        smeses.append(evaluator.smses[-1])
        rmses.append(evaluator.rmses[-1])
        maes.append(evaluator.maes[-1])
        mslls.append(evaluator.mslls[-1])
        nlpds.append(evaluator.nlpds[-1])
        training_times.append(evaluator.training_times[-1])
        prediction_times.append(evaluator.prediction_times[-1])
    ...
    print(f"Average smses: {np.mean(smeses)}")
    print(f"Average rmses: {np.mean(rmses)}")
    print(f"Average maes: {np.mean(maes)}")
    print(f"Average mslls: {np.mean(mslls)}")
    print(f"Average nlpds: {np.mean(nlpds)}")
    print(f"Average training time: {np.mean(training_times)} seconds")
    print(f"Average prediction time: {np.mean(prediction_times)} seconds")

    print("\n")

    # stdandard deviation
    print(f"Standard deviation of smses: {np.std(smeses)}")
    print(f"Standard deviation of rmses: {np.std(rmses)}")
    print(f"Standard deviation of maes: {np.std(maes)}")
    print(f"Standard deviation of mslls: {np.std(mslls)}")
    print(f"Standard deviation of nlpds: {np.std(nlpds)}")
    print(f"Standard deviation of training times: {np.std(training_times)} seconds")
    print(f"Standard deviation of prediction times: {np.std(prediction_times)} seconds")
