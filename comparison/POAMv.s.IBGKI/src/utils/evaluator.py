...
LastEditTime: 2025-05-19 06:13:09
FilePath: /POAM/src/utils/evaluator.py
Description: 
Reference: 
...
class Evaluator:
    def __init__(self, sensor, task_extent, eval_grid):
        ...
        num_x, num_y = eval_grid # eval_grid: [50, 50]
        ...
        print(f'self.eval_inputs.shape:{self.eval_inputs.shape}')
        self.eval_outputs = sensor.get( # get sensor sample elevation   
            self.eval_inputs[:, 0], self.eval_inputs[:, 1]
        ).reshape(-1, 1)
        self.eval_outputs_var = np.var(self.eval_outputs) # compute variance, noiesy'svariance
        self.num_samples = []
        self.smses = []
        self.rmses = []
        self.maes = []
